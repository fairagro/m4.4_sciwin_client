...
use commonwl::{StringOrDocument, Workflow};
...
pub fn get_all_outputs(workflow: &Workflow) -> Result<Vec<(String, String)>> {
    let mut results = Vec::new();
    for output in &workflow.outputs {
        let output_id = output.id.clone();
        let Some(source) = &output.output_source else {
            eprintln!("⚠️ Skipping output '{output_id}' — no outputSource defined");
            continue;
        };
        let parts: Vec<&str> = source.split('/').collect();
        if parts.len() != 2 {
            eprintln!("⚠️ Skipping invalid outputSource '{source}' for output '{output_id}'");
            continue;
        }
        let step_id = parts[0];
        let step_output_id = parts[1];
        if let Some(step) = workflow.steps.iter().find(|s| s.id == step_id) {
            if step.out.iter().any(|o| o == step_output_id) {
                results.push((output_id.clone(), format!("{step_id}/{step_output_id}")));
            } else {
                eprintln!("⚠️ Step '{step_id}' does not define output '{step_output_id}'");
            }
        } else {
            eprintln!("⚠️ Step '{step_id}' not found in workflow");
        }
    }
    if results.is_empty() {
...
mod tests {
   ...
   #[test]
    fn test_load_cwl_file_valid_main_cwl() -> anyhow::Result<()> {
        ...
        assert!(
            cwl_path.exists(),
            "Expected test CWL file to exist at {:?}",
            cwl_path
        );

        let result = load_cwl_file(
            base_dir.to_str().unwrap(), 
            Path::new("testdata/hello_world/workflows/main/main.cwl"),
        );
        ...
        assert_eq!(
            ...
            "Expected CWL class to be 'Workflow'"
        );
        assert!(
            yaml.get("inputs").is_some(),
            "Expected CWL file to contain 'inputs' section"
        );
        assert!(
            yaml.get("steps").is_some(),
            "Expected CWL file to contain 'steps' section"
        );
    ...
    fn test_get_all_outputs_with_existing_file() {
        ...
        let result = get_all_outputs(&workflow);
        assert!(result.is_ok());
        let outputs = result.unwrap();
        assert_eq!(outputs.len(), 1);
        assert_eq!(outputs[0], ("out".to_string(), "plot/o_results".to_string()));
    }
}
